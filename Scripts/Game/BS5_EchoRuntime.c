...
class BS5_NearReflectionProbe
{
	static BS5_NearReflectionSnapshot BuildFromLegacy(BS5_EchoDriverComponent settings, BS5_EchoAnalysisResult result, vector origin, vector flatForward, vector flatRight, array<ref BS5_EchoReflectorCandidate> wallCandidates, int wallHitCount, int rayCount, BS5_CloseReflectionPlannerResult closePlannerResult, bool explosionLike)
	{
		return BS5_EchoEnvironmentAnalyzer.BuildNearReflectionSnapshotFromLegacy(settings, result, origin, flatForward, flatRight, wallCandidates, wallHitCount, rayCount, closePlannerResult, explosionLike);
	}
}
...
class BS5_EchoEnvironmentAnalyzer
{
	...
	static BS5_EchoAnalysisResult Analyze(BS5_EchoDriverComponent settings, IEntity owner, vector origin, vector forward, bool explosionLike, bool suppressed)
	{
		...
		result.m_iNearTraceCount = traceLimit;
		...
		if (settings.IsDebugEnabled())
		{
			...
			debugSummary += " nearTraces=" + result.m_iNearTraceCount;
			debugSummary += " nearHits=" + result.m_iNearTraceHits;
			...
			debugSummary += " pathSamples=" + result.m_iSoundMapPathSamples;
			...
			debugSummary += " budgetSkips=" + result.m_iSoundMapBudgetSkips;
			...
			debugSummary += " slapEntityQueries=" + result.m_iSlapbackEntityQueryCount;
			debugSummary += " slapEntityCandidates=" + result.m_iSlapbackEntityCandidateCount;
			...
			if (result.m_NearReflection)
			{
				debugSummary += " nearMode=" + BS5_EchoMath.NearReflectionModeName(result.m_NearReflection.m_iImplementationMode);
				debugSummary += " nearScenario=" + BS5_EchoMath.NearReflectionScenarioName(result.m_NearReflection.m_eScenario);
				debugSummary += " nearTraces=" + result.m_NearReflection.m_iTraceCount;
				debugSummary += " nearHits=" + result.m_NearReflection.m_iTraceHits;
				debugSummary += " nearBest=" + result.m_NearReflection.m_fProbeBestCloseScore;
				debugSummary += " nearCache=" + result.m_NearReflection.m_bCacheHit;
			}
			...
			if (settings.IsDebugChannelEnabled(BS5_DebugChannel.SOUNDMAP))
			{
				...
				perfCounts += " nearTraces=" + result.m_iNearTraceCount;
				perfCounts += " nearHits=" + result.m_iNearTraceHits;
				...
				perfCounts += " pathSamples=" + result.m_iSoundMapPathSamples;
				perfCounts += " pathRaycasts=" + result.m_iSoundMapPathRaycasts;
				perfCounts += " budgetSkips=" + result.m_iSoundMapBudgetSkips;
				perfCounts += " slapRays=" + result.m_iSlapbackRayCount;
				perfCounts += " slapHits=" + result.m_iSlapbackHitCount;
	...
	protected static void CollectSlapbackCandidates(array<ref BS5_EchoReflectorCandidate> candidates, BS5_EchoDriverComponent settings, BS5_EchoAnalysisResult result, IEntity owner, vector origin, vector probeOrigin, vector forwardDir, vector right, array<IEntity> traceExcludeArray, bool explosionLike)
	{
		...
		BS5_NearReflectionSnapshot nearReflection = BS5_NearReflectionProbe.BuildFromLegacy(settings, result, origin, flatForward, flatRight, wallCandidates, hitCount, directions.Count(), null, explosionLike);
		if (result)
			result.m_NearReflection = nearReflection;
		BS5_CloseReflectionPlannerResult closePlannerResult = BS5_CloseReflectionPlanner.Evaluate(settings, closeSettings, result, origin, slapbackProbeOrigin, flatForward, flatRight, wallCandidates, wallHitCount, directions.Count(), rawCoverageMask, trenchCandidate, traceExcludeArray, traceExcludeRoot, nearReflection);
		if (nearReflection && closePlannerResult)
			nearReflection.m_fLegacyBestCloseScore = closePlannerResult.m_fScore;
		float closeScore = 0.0;
		bool closeAccepted = false;
		bool nearLayerAccepted = false;
		...
		if (closePlannerResult && closePlannerResult.m_bAccepted && closePlannerResult.m_Candidate)
		{
			...
			bool canInsertCloseCandidate = settings.GetNearReflectionMaxDirectionalCloseCandidates() > 0;
			if (explosionLike)
				canInsertCloseCandidate = settings.GetExplosionNearReflectionMaxCloseCandidatesPerShot() > 0;
			if (!trenchDominatesClose && canInsertCloseCandidate)
			{
				int closeInsertLimit = 1;
				if (explosionLike)
					closeInsertLimit = Math.Clamp(settings.GetExplosionNearReflectionMaxCloseCandidatesPerShot(), 1, maxCandidates);
				InsertCandidate(candidates, closePlannerResult.m_Candidate, closeInsertLimit);
				closeAccepted = !candidates.IsEmpty();
				if (closeAccepted && closeSettings)
				{
					if (explosionLike)
						candidateLimit = Math.Clamp(settings.GetExplosionNearReflectionMaxCloseCandidatesPerShot(), 1, maxCandidates);
					else
		...
		bool trenchPreferred = trenchCandidate && trenchScore >= settings.GetTrenchSlapbackMinConfidence();
		if (trenchPreferred && closeAccepted && !trenchDominatesClose)
			trenchPreferred = false;
		if (!explosionLike && !trenchPreferred && closeSettings && closeSettings.IsEnabled())
		{
		...
		if (!closeAccepted && !nearLayerAccepted && trenchCandidate && trenchScore >= settings.GetTrenchSlapbackMinConfidence())
		{
		...
		for (int winnerIndex = 0; winnerIndex < wallCandidates.Count() && candidates.Count() < candidateLimit; winnerIndex++)
		{
			...
			for (int existingIndex = 0; existingIndex < candidates.Count(); existingIndex++)
			{
				...
				if (existing.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_CORE
					|| existing.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR
					|| existing.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_EXPLOSION_CLOSE)
				{
					merged = true;
					break;
				}
		...
		float nearLayerScore = 0.0;
		string nearLayerType = "none";
		float reportedCloseScore = closeScore;
		bool reportedCloseAccepted = closeAccepted;
		if (nearLayerAccepted)
		{
			nearLayerScore = ResolveBestCloseLayerCandidateScore(candidates);
			nearLayerType = ResolveBestCloseLayerCandidateSourceName(candidates);
			if (nearLayerScore > reportedCloseScore)
				reportedCloseScore = nearLayerScore;
			if (nearLayerScore > 0.0)
				reportedCloseAccepted = true;
		}
		string reportedCloseSource = "none";
		if (reportedCloseAccepted)
		{
			if (nearLayerAccepted && nearLayerScore > 0.0 && nearLayerScore >= closeScore)
				reportedCloseSource = "near_layer";
			else if (closeAccepted)
				reportedCloseSource = "planner";
			else if (nearLayerAccepted && nearLayerScore > 0.0)
				reportedCloseSource = "near_layer";
		}
		...
		if (result)
		{
			...
			result.m_iSlapbackEntityQueryCount = entityQueryCount;
			result.m_iSlapbackEntityCandidateCount = entityHitCount;
			result.m_bSlapbackAnchorFallback = anchorFallback;
			result.m_fSlapbackWallScore = bestWallScore;
			result.m_fSlapbackCloseScore = reportedCloseScore;
			...
			slapbackDebugSummary += " entityQueries=" + entityQueryCount;
			...
			if (nearReflection)
			{
				slapbackDebugSummary += " nearMode=" + BS5_EchoMath.NearReflectionModeName(nearReflection.m_iImplementationMode);
				slapbackDebugSummary += " nearScenario=" + BS5_EchoMath.NearReflectionScenarioName(nearReflection.m_eScenario);
				slapbackDebugSummary += " nearHits=" + nearReflection.m_iTraceHits;
				slapbackDebugSummary += " nearBest=" + nearReflection.m_fProbeBestCloseScore;
			}
			...
			closeDebugSummary += " accepted=" + BS5_DebugLog.BoolText(reportedCloseAccepted);
			closeDebugSummary += " score=" + reportedCloseScore;
			closeDebugSummary += " closeSource=" + reportedCloseSource;
			closeDebugSummary += " layerType=" + nearLayerType;
			...
			if (nearReflection)
			{
				closeDebugSummary += " nearMode=" + BS5_EchoMath.NearReflectionModeName(nearReflection.m_iImplementationMode);
				closeDebugSummary += " nearScenario=" + BS5_EchoMath.NearReflectionScenarioName(nearReflection.m_eScenario);
				closeDebugSummary += " nearDominant=" + nearReflection.m_iDominantSector;
				closeDebugSummary += " nearReflect=" + nearReflection.m_fDominantReflectWeight;
				...
				closeDebugSummary += " nearFacadeShell=" + nearReflection.m_fFacadeShellEvidence;
				closeDebugSummary += " nearSemiSource=" + ResolveNearReflectionSemiSourceName(nearReflection.m_iSemiIndoorSource);
		...
		if (settings && settings.IsDebugChannelEnabled(BS5_DebugChannel.SLAPBACK))
		{
			...
			if (nearReflection)
			{
				slapCollectLog += " nearMode=" + BS5_EchoMath.NearReflectionModeName(nearReflection.m_iImplementationMode);
				slapCollectLog += " nearScenario=" + BS5_EchoMath.NearReflectionScenarioName(nearReflection.m_eScenario);
			}
			...
			slapScoreLog += " closeScore=" + reportedCloseScore;
			slapScoreLog += " trenchScore=" + trenchScore;
			slapScoreLog += " closeAccepted=" + BS5_DebugLog.BoolText(reportedCloseAccepted);
		...
		if (settings && settings.IsDebugChannelEnabled(BS5_DebugChannel.CLOSE))
		{
			...
			closeLog += " accepted=" + BS5_DebugLog.BoolText(reportedCloseAccepted);
			closeLog += " score=" + reportedCloseScore;
			closeLog += " closeSource=" + reportedCloseSource;
			closeLog += " layerType=" + nearLayerType;
			closeLog += " trenchScore=" + trenchScore;
			closeLog += " wallCandidates=" + wallCandidates.Count();
			if (nearReflection)
			{
				closeLog += " nearMode=" + BS5_EchoMath.NearReflectionModeName(nearReflection.m_iImplementationMode);
				closeLog += " nearScenario=" + BS5_EchoMath.NearReflectionScenarioName(nearReflection.m_eScenario);
				closeLog += " nearDominant=" + nearReflection.m_iDominantSector;
				closeLog += " nearReflect=" + nearReflection.m_fDominantReflectWeight;
				...
				closeLog += " nearFacadeShell=" + nearReflection.m_fFacadeShellEvidence;
				closeLog += " nearSemiSource=" + ResolveNearReflectionSemiSourceName(nearReflection.m_iSemiIndoorSource);
	...
	protected static float ResolveBestCloseLayerCandidateScore(notnull array<ref BS5_EchoReflectorCandidate> candidates)
	{
		float bestScore = 0.0;
		for (int candidateIndex = 0; candidateIndex < candidates.Count(); candidateIndex++)
		{
			...
			if (candidate.m_eSourceType != BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_CORE
				&& candidate.m_eSourceType != BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR
				&& candidate.m_eSourceType != BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_SPACE
				&& candidate.m_eSourceType != BS5_EchoCandidateSourceType.SLAPBACK_EXPLOSION_CLOSE)
				continue;
	...
	protected static string ResolveBestCloseLayerCandidateSourceName(notnull array<ref BS5_EchoReflectorCandidate> candidates)
	{
		float bestScore = 0.0;
		...
		for (int candidateIndex = 0; candidateIndex < candidates.Count(); candidateIndex++)
		{
			...
			if (candidate.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_CORE)
				candidateSource = "close_core";
			else if (candidate.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR)
				candidateSource = "semi_indoor";
			else if (candidate.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_SPACE)
				candidateSource = "close";
			else if (candidate.m_eSourceType == BS5_EchoCandidateSourceType.SLAPBACK_EXPLOSION_CLOSE)
				candidateSource = "explosion_close";
			else
				continue;
	...
	protected static string ResolveNearReflectionSemiSourceName(int source)
	{
		if (source == 1)
			return "canopy_roof";
		if (source == 2)
	...
	protected static bool TryInsertNearReflectionLayerCandidate(notnull array<ref BS5_EchoReflectorCandidate> candidates, BS5_EchoDriverComponent settings, BS5_CloseReflectionSettingsComponent closeSettings, BS5_EchoAnalysisResult result, vector origin, vector flatForward, BS5_NearReflectionSnapshot nearReflection, int candidateLimit)
	{
		...
		bool suppressed = result.m_bSuppressedShot;
		float thresholdOffset = 0.0;
		if (suppressed)
			thresholdOffset = 0.08;

		BS5_EchoCandidateSourceType sourceType = BS5_EchoCandidateSourceType.UNKNOWN;
		float score = 0.0;
		float support = 0.0;
		if (settings.GetNearReflectionMaxSemiIndoorCandidatesPerShot() > 0 && nearReflection.m_eScenario == BS5_NearReflectionScenario.SEMI_INDOOR)
		{
			score = BS5_EchoMath.Clamp01((nearReflection.m_fSemiIndoorWeight * 0.56) + (nearReflection.m_fConfinementScore * 0.26) + (nearReflection.m_fDominantReflectWeight * 0.18));
			support = nearReflection.m_fSemiIndoorWeight;
			if (score >= closeSettings.GetAcceptScoreMin() + thresholdOffset)
				sourceType = BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR;
		}
		...
		if (sourceType == BS5_EchoCandidateSourceType.UNKNOWN && settings.GetNearReflectionMaxCoreCandidatesPerShot() > 0)
		{
			...
			bool broadCorridorCore = nearReflection.m_eScenario == BS5_NearReflectionScenario.CORRIDOR_ALLEY
				&& nearReflection.m_iLegacyAcceptedCount >= 3
				&& nearReflection.m_fConfinementScore >= 0.86
				&& nearReflection.m_fPairEvidence >= 0.62
				&& nearReflection.m_fCoreCloseWeight >= 0.70
				&& nearReflection.m_fDominantReflectWeight >= 0.68;
			bool broadCornerCore = nearReflection.m_eScenario == BS5_NearReflectionScenario.CORNER
				&& nearReflection.m_iLegacyAcceptedCount >= 3
				&& nearReflection.m_fConfinementScore >= 0.84
		...
		int oldCount = candidates.Count();
		InsertCandidate(candidates, layerCandidate, candidateLimit);
	...
	protected static BS5_EchoReflectorCandidate BuildNearReflectionLayerCandidate(BS5_EchoDriverComponent settings, vector origin, vector flatForward, BS5_EchoCandidateSourceType sourceType, float score, float support, BS5_NearReflectionSnapshot nearReflection)
	{
		...
		vector direction = flatForward;
		direction[1] = 0.0;
		if (direction.LengthSq() < 0.0001)
			direction = "0 0 1";
		direction.Normalize();

		float distance = settings.GetSlapbackMinDistanceMeters();
		BS5_EchoReflectorCandidate candidate = new BS5_EchoReflectorCandidate();
		candidate.m_bValid = true;
		...
		candidate.m_fDistance = distance;
		candidate.m_fScore = BS5_EchoMath.Clamp01(score);
		candidate.m_fDelaySeconds = settings.GetSlapbackDelayMinSeconds();
		candidate.m_fDistanceNorm = 0.0;
		candidate.m_fPanBias = 0.0;
		candidate.m_fDirectionSupport = BS5_EchoMath.Clamp01(support);
		candidate.m_fPhysicalScore = BS5_EchoMath.Clamp01(support);
		candidate.m_fZonePriority = 1.0;
		candidate.m_fPathPlausibility = 1.0;
		candidate.m_iRank = -20;
		if (sourceType == BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR)
			candidate.m_iRank = -24;
	...
	static BS5_NearReflectionSnapshot BuildNearReflectionSnapshotFromLegacy(BS5_EchoDriverComponent settings, BS5_EchoAnalysisResult result, vector origin, vector flatForward, vector flatRight, array<ref BS5_EchoReflectorCandidate> wallCandidates, int wallHitCount, int rayCount, BS5_CloseReflectionPlannerResult closePlannerResult, bool explosionLike)
	{
		...
		int mode = settings.GetNearReflectionImplementationMode();
		if (mode == BS5_NearReflectionImplementationMode.LEGACY_ONLY)
			return null;

		BS5_NearReflectionSnapshot snapshot = new BS5_NearReflectionSnapshot();
		snapshot.m_bValid = true;
		snapshot.m_bExplosionProfile = explosionLike;
		snapshot.m_iImplementationMode = mode;
		snapshot.m_iTraceCount = rayCount;
		snapshot.m_iTraceHits = wallHitCount;
		snapshot.m_iWallHits = wallHitCount;
		snapshot.m_iLegacyTraceCount = rayCount;
		snapshot.m_iLegacyAcceptedCount = 0;
		snapshot.m_fSectorOcclusionStrength = settings.GetNearReflectionSectorOcclusionStrength(explosionLike);
		...
		if (wallCandidates)
		{
			float maxDistance = settings.GetNearReflectionMaxDistanceMeters(explosionLike);
			float tightDistance = settings.GetNearReflectionTightDistanceMeters(explosionLike);
			for (int i = 0; i < wallCandidates.Count(); i++)
			{
				...
				snapshot.m_iLegacyAcceptedCount++;
				int sectorIndex = ResolveNearReflectionSector(origin, flatForward, flatRight, candidate.m_vPosition);
				if (sectorIndex < 0 || sectorIndex >= snapshot.m_aSectors.Count())
					continue;
				...
				float distanceFit = 1.0 - BS5_EchoMath.Clamp01((candidate.m_fDistance - settings.GetSlapbackMinDistanceMeters()) / BS5_EchoMath.MaxFloat(0.1, maxDistance - settings.GetSlapbackMinDistanceMeters()));
				float tightFit = 1.0 - BS5_EchoMath.Clamp01(candidate.m_fDistance / BS5_EchoMath.MaxFloat(0.1, tightDistance));
				float reflectWeight = BS5_EchoMath.Clamp01((candidate.m_fScore * 0.58) + (distanceFit * 0.27) + (tightFit * 0.15));

				if (!sectorData.m_bHasHit || reflectWeight > sectorData.m_fReflectWeight)
				{
					sectorData.m_bHasHit = true;
					sectorData.m_fReflectWeight = reflectWeight;
					sectorData.m_fOcclusionWeight = BS5_EchoMath.Clamp01(reflectWeight * snapshot.m_fSectorOcclusionStrength);
					sectorData.m_fNearestDistance = candidate.m_fDistance;
					sectorData.m_fWallEvidence = reflectWeight;
					sectorData.m_vHitPosition = candidate.m_vPosition;
					sectorData.m_vHitNormal = candidate.m_vNormal;
				}

				if (reflectWeight > snapshot.m_fDominantReflectWeight)
				{
					snapshot.m_fDominantReflectWeight = reflectWeight;
					snapshot.m_iDominantSector = sectorIndex;
				}
			}
		}

		if (snapshot.m_iLegacyAcceptedCount > snapshot.m_iTraceHits)
			snapshot.m_iTraceHits = snapshot.m_iLegacyAcceptedCount;
		if (snapshot.m_iLegacyAcceptedCount > snapshot.m_iWallHits)
			snapshot.m_iWallHits = snapshot.m_iLegacyAcceptedCount;

		snapshot.m_fProbeBestCloseScore = snapshot.m_fDominantReflectWeight;
		FinalizeNearReflectionSnapshot(snapshot, result);
		return snapshot;
	}

	protected static int ResolveNearReflectionSector(vector origin, vector flatForward, vector flatRight, vector hitPosition)
	{
		vector direction = hitPosition - origin;
		direction[1] = 0.0;
		if (direction.LengthSq() < 0.0001)
			return BS5_NearReflectionSector.FRONT;
		direction.Normalize();

		float forwardDot = vector.Dot(direction, flatForward);
		float sideDot = vector.Dot(direction, flatRight);
		float absForward = Math.AbsFloat(forwardDot);
		float absSide = Math.AbsFloat(sideDot);

		if (forwardDot >= 0.45 && absSide < 0.35)
			return BS5_NearReflectionSector.FRONT;
		if (forwardDot <= -0.45 && absSide < 0.35)
			return BS5_NearReflectionSector.BACK;
		if (sideDot <= -0.45 && absForward < 0.35)
			return BS5_NearReflectionSector.LEFT;
		if (sideDot >= 0.45 && absForward < 0.35)
			return BS5_NearReflectionSector.RIGHT;
		if (forwardDot >= 0.0 && sideDot < 0.0)
			return BS5_NearReflectionSector.FRONT_LEFT;
		if (forwardDot >= 0.0 && sideDot >= 0.0)
			return BS5_NearReflectionSector.FRONT_RIGHT;
		if (forwardDot < 0.0 && sideDot < 0.0)
			return BS5_NearReflectionSector.BACK_LEFT;

		return BS5_NearReflectionSector.BACK_RIGHT;
	}

	protected static void FinalizeNearReflectionSnapshot(BS5_NearReflectionSnapshot snapshot, BS5_EchoAnalysisResult result)
	{
		if (!snapshot)
			return;

		float leftEvidence = GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.LEFT);
		leftEvidence = BS5_EchoMath.MaxFloat(leftEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_LEFT));
		leftEvidence = BS5_EchoMath.MaxFloat(leftEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_LEFT));
		float rightEvidence = GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.RIGHT);
		rightEvidence = BS5_EchoMath.MaxFloat(rightEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_RIGHT));
		rightEvidence = BS5_EchoMath.MaxFloat(rightEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_RIGHT));
		float frontEvidence = GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT);
		frontEvidence = BS5_EchoMath.MaxFloat(frontEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_LEFT));
		frontEvidence = BS5_EchoMath.MaxFloat(frontEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_RIGHT));
		float backEvidence = GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK);
		backEvidence = BS5_EchoMath.MaxFloat(backEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_LEFT));
		backEvidence = BS5_EchoMath.MaxFloat(backEvidence, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_RIGHT));

		float sidePair = BS5_EchoMath.MinFloat(leftEvidence, rightEvidence);
		float frontBackPair = BS5_EchoMath.MinFloat(frontEvidence, backEvidence);
		snapshot.m_fPairEvidence = BS5_EchoMath.MaxFloat(sidePair, frontBackPair);
		snapshot.m_fCornerEvidence = ResolveNearReflectionCornerEvidence(snapshot);
		...
		snapshot.m_fFacadeShellEvidence = facadeShellEvidence;
		...
		bool pairedCanopyBackedSemi = snapshot.m_fCanopyEvidence > 0.35 && snapshot.m_fPairEvidence > 0.25;
		bool edgeCanopyBackedSemi = snapshot.m_iRoofHits >= 1
			&& roofEvidence >= 0.72
			&& snapshot.m_fCanopyEvidence >= 0.72
			&& snapshot.m_iLegacyAcceptedCount >= 4
			&& traceDensity >= 0.80
			&& snapshot.m_fConfinementScore >= 0.58
			&& snapshot.m_fDominantReflectWeight >= 0.70
			&& facadeShellEvidence < 0.35;
		bool canopyBackedSemi = pairedCanopyBackedSemi || edgeCanopyBackedSemi;
		bool denseFacadeBackedSemi = facadeShellEvidence >= 0.55
			&& snapshot.m_iLegacyAcceptedCount >= 3
			&& snapshot.m_fConfinementScore >= 0.72
			&& (traceDensity >= 0.30 || snapshot.m_fPairEvidence >= 0.48);
		bool sparseFacadeShellSemi = facadeShellEvidence >= 0.78
			&& snapshot.m_iLegacyAcceptedCount >= 2
			&& traceDensity <= 0.35
			&& snapshot.m_fConfinementScore >= 0.72
			&& snapshot.m_fDominantReflectWeight >= 0.78
			&& (snapshot.m_fPairEvidence >= 0.34 || snapshot.m_fCoreCloseWeight >= 0.52);
		bool facadeBackedSemi = denseFacadeBackedSemi || sparseFacadeShellSemi;
		if (canopyBackedSemi || facadeBackedSemi)
		{
			snapshot.m_fSemiIndoorWeight = snapshot.m_fSemiIndoorEvidence;
			if (canopyBackedSemi && facadeBackedSemi)
				snapshot.m_iSemiIndoorSource = 3;
			else if (facadeBackedSemi)
				snapshot.m_iSemiIndoorSource = 2;
			else
				snapshot.m_iSemiIndoorSource = 1;
		}
		else
		{
			snapshot.m_fSemiIndoorWeight = 0.0;
			snapshot.m_iSemiIndoorSource = 0;
		}

		if (snapshot.m_bExplosionProfile && snapshot.m_fConfinementScore >= 0.42)
			snapshot.m_eScenario = BS5_NearReflectionScenario.EXPLOSION_CONFINED;
		else if (snapshot.m_fSemiIndoorWeight >= 0.55)
			snapshot.m_eScenario = BS5_NearReflectionScenario.SEMI_INDOOR;
		else if (snapshot.m_fCanopyEvidence >= 0.55 && snapshot.m_fDominantReflectWeight < 0.30)
			snapshot.m_eScenario = BS5_NearReflectionScenario.CANOPY;
		else if (snapshot.m_fPairEvidence >= 0.48 && (frontEvidence > 0.25 || backEvidence > 0.25))
			snapshot.m_eScenario = BS5_NearReflectionScenario.CORRIDOR_ALLEY;
		else if (snapshot.m_fPairEvidence >= 0.42)
			snapshot.m_eScenario = BS5_NearReflectionScenario.SIDE_PAIR;
		else if (snapshot.m_fCornerEvidence >= 0.40)
			snapshot.m_eScenario = BS5_NearReflectionScenario.CORNER;
		else if (snapshot.m_fDominantReflectWeight >= 0.28)
	...
	protected static float GetNearReflectionSectorWallEvidence(BS5_NearReflectionSnapshot snapshot, int sectorIndex)
	{
		...
		return sectorData.m_fWallEvidence;
	}

	protected static float ResolveNearReflectionCornerEvidence(BS5_NearReflectionSnapshot snapshot)
	{
		float bestCorner = 0.0;
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, BS5_EchoMath.MinFloat(GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT), GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.LEFT)));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, BS5_EchoMath.MinFloat(GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT), GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.RIGHT)));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, BS5_EchoMath.MinFloat(GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK), GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.LEFT)));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, BS5_EchoMath.MinFloat(GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK), GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.RIGHT)));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_LEFT));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.FRONT_RIGHT));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_LEFT));
		bestCorner = BS5_EchoMath.MaxFloat(bestCorner, GetNearReflectionSectorWallEvidence(snapshot, BS5_NearReflectionSector.BACK_RIGHT));
		return BS5_EchoMath.Clamp01(bestCorner);
	}
	...
	protected static void CollectSlapbackEntityCandidates(array<ref BS5_EchoReflectorCandidate> wallCandidates, BS5_EchoDriverComponent settings, BS5_EchoAnalysisResult result, BaseWorld world, vector origin, vector probeOrigin, vector flatForward, vector flatRight, IEntity owner, array<vector> directions, float maxDistance, float minDistance, int maxCandidates, out int entityHits, out float bestScore, out int queryCount)
	{
		...
		int newEntityHits;
		float queryBestScore;
		int processedEntityCount = s_SlapbackQueryEntities.Count();
		ProcessSlapbackEntityQueryRange(wallCandidates, settings, origin, flatForward, flatRight, maxDistance, minDistance, 0, newEntityHits, queryBestScore);
		entityHits += newEntityHits;
		if (queryBestScore > bestScore)
		...
		for (int directionIndex = 0; directionIndex < directions.Count(); directionIndex++)
		{
			...
			ProcessSlapbackEntityQueryRange(wallCandidates, settings, origin, flatForward, flatRight, maxDistance, minDistance, processedEntityCount, newEntityHits, queryBestScore);
			processedEntityCount = s_SlapbackQueryEntities.Count();
			entityHits += newEntityHits;
			if (queryBestScore > bestScore)
				bestScore = queryBestScore;
	...
	protected static void ProcessSlapbackEntityQueryRange(array<ref BS5_EchoReflectorCandidate> wallCandidates, BS5_EchoDriverComponent settings, vector origin, vector flatForward, vector flatRight, float maxDistance, float minDistance, int startIndex, out int entityHits, out float bestScore)
	{
		entityHits = 0;
		bestScore = 0.0;
		if (!s_SlapbackQueryEntities)
		...
		for (int entityIndex = startIndex; entityIndex < s_SlapbackQueryEntities.Count(); entityIndex++)
		{
	...
	protected static bool IsSlapbackEntityQuerySatisfied(array<ref BS5_EchoReflectorCandidate> wallCandidates, int entityHits, float bestScore, int maxCandidates)
	{
		if (maxCandidates < 1)
			maxCandidates = 1;
		int readyCandidateLimit = maxCandidates;
		if (readyCandidateLimit > 4)
			readyCandidateLimit = 4;

		if (entityHits < readyCandidateLimit)
			return false;
		if (!wallCandidates || wallCandidates.Count() < readyCandidateLimit)
			return false;

		return bestScore >= SLAPBACK_ENTITY_LINE_QUERY_READY_SCORE;
	}
...
class BS5_EchoEmissionService
{
	...
	protected static ref array<string> s_aInvalidSoundManagerPlaybackKeys;
	...
	protected static const int INVALID_SOUNDMANAGER_PLAYBACK_CACHE_LIMIT = 32;
	protected static const int INVALID_SOUNDMANAGER_PLAYBACK_CACHE_TTL_MS = 15000;
	protected static const int PENDING_TAIL_DUPLICATE_LIMIT = 2;
	protected static const float PENDING_TAIL_DUPLICATE_POSITION_METERS = 5.0;
	protected static const float PENDING_TAIL_DUPLICATE_DELAY_SECONDS = 0.08;
	protected static const float PENDING_TAIL_DUPLICATE_SOFT_FAR_DISTANCE_METERS = 140.0;
	protected static const float PENDING_TAIL_DUPLICATE_SOFT_LOW_INTENSITY = 0.46;

	static string BuildLimiterPerfSnapshot(BS5_EchoDriverComponent settings)
	{
		...
		int activeTail = 0;
		int activeSlapback = 0;
		for (int activeIndex = 0; activeIndex < s_aActiveVoices.Count(); activeIndex++)
		{
			BS5_ActiveEchoVoice voice = s_aActiveVoices[activeIndex];
			if (!voice || !voice.m_Context || voice.m_Context.m_bEmitterCleanupDone)
				continue;

			if (voice.m_bSlapback)
				activeSlapback += GetContextVoiceWeight(voice.m_Context);
			else
				activeTail += GetContextVoiceWeight(voice.m_Context);
		}

		int reservedTail = 0;
		int reservedSlapback = 0;
		for (int reservedIndex = 0; reservedIndex < s_aReservedPlaybackVoices.Count(); reservedIndex++)
		{
			...
			if (reserved.m_bSlapback)
				reservedSlapback += GetContextVoiceWeight(reserved);
			else
				reservedTail += GetContextVoiceWeight(reserved);
		}

		string snapshot = "activeTail=" + activeTail;
		snapshot += " activeSlap=" + activeSlapback;
		snapshot += " reservedTail=" + reservedTail;
		snapshot += " reservedSlap=" + reservedSlapback;
		snapshot += " pendingTail=" + s_iPendingTailVoices;
		snapshot += " pendingSlap=" + s_iPendingSlapbackVoices;
	...
	static bool QueueEmission(IEntity owner, BS5_EchoAnalysisResult result, ResourceName project, string eventName, BS5_EchoReflectorCandidate candidate, bool slapback, bool explosionLike, BS5_EchoDriverComponent settings)
	{
		...
		if (ShouldSkipPendingTailDuplicate(context, settings))
			return false;
	...
	protected static bool ShouldSkipPendingTailDuplicate(BS5_PendingEmissionContext context, BS5_EchoDriverComponent settings)
	{
		if (!context || !settings || !settings.IsPlaybackLimiterEnabled())
			return false;
		if (context.m_bSlapback || context.m_bExplosion)
			return false;
		if (!s_aPendingEmissionContexts)
			return false;

		float positionDistanceSq = PENDING_TAIL_DUPLICATE_POSITION_METERS * PENDING_TAIL_DUPLICATE_POSITION_METERS;
		int duplicateLimit = GetPendingTailDuplicateLimit(context);
		int duplicateCount = 0;
		for (int pendingIndex = 0; pendingIndex < s_aPendingEmissionContexts.Count(); pendingIndex++)
		{
			BS5_PendingEmissionContext pending = s_aPendingEmissionContexts[pendingIndex];
			if (!pending || pending.m_bEmitterCleanupDone || pending.m_bCancelled)
				continue;
			if (pending.m_bSlapback || pending.m_bExplosion)
				continue;
			if (!ContextMatchesOwner(pending, context.m_pOwner, context.m_OwnerId))
				continue;
			if (pending.m_sProject != context.m_sProject || pending.m_sEventName != context.m_sEventName || pending.m_eSourceType != context.m_eSourceType)
				continue;
			if (vector.DistanceSq(pending.m_vEmitPosition, context.m_vEmitPosition) > positionDistanceSq)
				continue;
			if (Math.AbsFloat(pending.m_fDelaySeconds - context.m_fDelaySeconds) > PENDING_TAIL_DUPLICATE_DELAY_SECONDS)
				continue;
			...
			if (settings.IsDebugChannelEnabled(BS5_DebugChannel.LIMITER))
			{
				string duplicateLog = "queue skip tail duplicate";
				duplicateLog += " existing=" + duplicateCount;
				duplicateLog += " limit=" + duplicateLimit;
				duplicateLog += " source=" + BS5_EchoMath.CandidateSourceName(context.m_eSourceType);
				duplicateLog += " delay=" + context.m_fDelaySeconds;
				duplicateLog += " dist=" + context.m_fCandidateDistance;
				duplicateLog += " pos=" + context.m_vEmitPosition;
				duplicateLog += " pendingTail=" + s_iPendingTailVoices;
				BS5_DebugLog.Channel(settings, BS5_DebugChannel.LIMITER, duplicateLog);
			}
	...
	protected static int GetPendingTailDuplicateLimit(BS5_PendingEmissionContext context)
	{
		if (!context)
			return PENDING_TAIL_DUPLICATE_LIMIT;

		if (!IsSoftTailDuplicateSource(context.m_eSourceType))
			return PENDING_TAIL_DUPLICATE_LIMIT;

		if (context.m_fCandidateDistance >= PENDING_TAIL_DUPLICATE_SOFT_FAR_DISTANCE_METERS)
			return 1;
		if (context.m_fIntensity <= PENDING_TAIL_DUPLICATE_SOFT_LOW_INTENSITY)
			return 1;

		return PENDING_TAIL_DUPLICATE_LIMIT;
	}

	protected static bool IsSoftTailDuplicateSource(BS5_EchoCandidateSourceType sourceType)
	{
		return sourceType == BS5_EchoCandidateSourceType.SOUNDMAP_CITY
			|| sourceType == BS5_EchoCandidateSourceType.SOUNDMAP_FOREST
			|| sourceType == BS5_EchoCandidateSourceType.SOUNDMAP_MEADOW
			|| sourceType == BS5_EchoCandidateSourceType.TERRAIN_RIDGE
			|| sourceType == BS5_EchoCandidateSourceType.FORWARD_FALLBACK
			|| sourceType == BS5_EchoCandidateSourceType.OMNI_CONTEXT;
	}
	...
	protected static float ComputeManagedEmitterLifetime(BS5_EchoDriverComponent settings, BS5_PendingEmissionContext context)
	{
		...
		{
			if (context.m_bSlapback)
				lifetime = settings.GetExplosionSlapbackEmitterLifetimeSeconds();
			else
	...
	protected static bool TryPlayManagedAudioSourceEvent(BS5_PendingEmissionContext context, SCR_SoundManagerModule soundManager, string eventName, bool debugEnabled)
	{
		...
		{
			BS5_DebugLog.OnceEnabled(debugEnabled, BS5_DebugChannel.EMIT, "soundmanager-play-skip-cached|" + playbackKey, "SoundManager skip cached failed playback project=" + context.m_sProject + " event=" + eventName + " slapback=" + BS5_DebugLog.BoolText(context.m_bSlapback));
			return false;
		}
	...
	protected static string BuildSoundManagerPlaybackKey(ResourceName project, string eventName, bool slapback)
	{
		return project + "|" + eventName + "|" + BS5_DebugLog.BoolText(slapback);
	}

	protected static bool IsSoundManagerPlaybackKnownInvalid(string playbackKey)
	{
		EnsureSoundManagerPlaybackCache();
		for (int invalidIndex = 0; invalidIndex < s_aInvalidSoundManagerPlaybackKeys.Count(); invalidIndex++)
		{
			if (s_aInvalidSoundManagerPlaybackKeys[invalidIndex] == playbackKey)
				return true;
		}
	...
	protected static void CacheInvalidSoundManagerPlayback(string playbackKey)
	{
		EnsureSoundManagerPlaybackCache();
		for (int invalidIndex = 0; invalidIndex < s_aInvalidSoundManagerPlaybackKeys.Count(); invalidIndex++)
		{
		...
		if (s_aInvalidSoundManagerPlaybackKeys.Count() >= INVALID_SOUNDMANAGER_PLAYBACK_CACHE_LIMIT)
			s_aInvalidSoundManagerPlaybackKeys.Remove(0);
		s_aInvalidSoundManagerPlaybackKeys.Insert(playbackKey);

		ScriptCallQueue callQueue = null;
		if (GetGame())
			callQueue = GetGame().GetCallqueue();
		if (callQueue)
			callQueue.CallLater(BS5_EchoEmissionService.ForgetInvalidSoundManagerPlayback, INVALID_SOUNDMANAGER_PLAYBACK_CACHE_TTL_MS, false, playbackKey);
		else
			ForgetInvalidSoundManagerPlayback(playbackKey);
	}

	protected static void ForgetInvalidSoundManagerPlayback(string playbackKey)
	{
		EnsureSoundManagerPlaybackCache();
		for (int invalidIndex = 0; invalidIndex < s_aInvalidSoundManagerPlaybackKeys.Count(); invalidIndex++)
		{
			...
			s_aInvalidSoundManagerPlaybackKeys.Remove(invalidIndex);
			return;
		}
	}

	protected static void EnsureSoundManagerPlaybackCache()
	{
		if (!s_aInvalidSoundManagerPlaybackKeys)
			s_aInvalidSoundManagerPlaybackKeys = new array<string>();
	}
	...
	{
		if (IsCloseReflectionSlapbackSource(sourceType))
			return BS5_PlayerAudioSettings.GetSlapbackCloseVolume();

		return BS5_PlayerAudioSettings.GetSlapbackVolume();
	}

	protected static bool IsCloseReflectionSlapbackSource(BS5_EchoCandidateSourceType sourceType)
	{
		return sourceType == BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_SPACE
			|| sourceType == BS5_EchoCandidateSourceType.SLAPBACK_CLOSE_CORE
			|| sourceType == BS5_EchoCandidateSourceType.SLAPBACK_SEMI_INDOOR
			|| sourceType == BS5_EchoCandidateSourceType.SLAPBACK_EXPLOSION_CLOSE;
	}
